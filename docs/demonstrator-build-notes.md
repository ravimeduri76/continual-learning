...
embeddings the top/bottom pick is right ~90–98% of the time for shapes/flags (`predict_demo.js`: 91/77% liked; 60 business leaders 57% over a 40% base, by
...
shapes/fill     0.327 / -0.322  gap 0.650      people/industry 0.496 / -0.138  gap 0.633
                                               people/region   0.310 / -0.407  gap 0.716
```
(People are 60 widely-recognisable contemporary business leaders; the prompt carries an
industry cue, so `industry` and `region` separate cleanly. No divisiveness label is
modelled or shown — it would prime the player's rating.)
...
- **Business leaders** (60 recognisable names) — tech-vs-not 89%, region Americas 92% / Asia 44%;
  sparse industries (finance) lift less. The set is a deliberate admired/divisive mix so the base
  admire-rate is ~40% — opinion splits, which is the point. Two curation rules earn their keep:
  prune to names a player will recognise (else the rating is a coin flip), and never show a
  "polarising" label (it would prime the very judgement being measured).
